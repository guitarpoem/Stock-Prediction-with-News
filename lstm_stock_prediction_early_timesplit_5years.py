...
def load_data(file_path, sequence_length=5):
    ...
    # Calculate daily differences for numerical features
    numerical_features = ['Open', 'High', 'Low', 'Close', 'Volume']
    for feature in numerical_features:
        df[f'{feature}_diff'] = df[feature].diff()
    
    # Add the difference features to our feature list
    diff_features = [f'{f}_diff' for f in numerical_features]
    features.extend(diff_features)
    
    # Drop the first row since it will have NaN for differences
    df = df.dropna()
...
def main():
    ...
    X, y, scaler = load_data(f'combined_{stock_name}.csv', sequence_length)
