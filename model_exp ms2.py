...
def run_experiment(use_sentiment, stock_name, num_trials=5):
    results = {
        'train_acc': [],
        'test_acc': []
    }
...
def main():
    ...
    num_trials = 10  # Set K=10
    
    for stock_name in stocks:
        print(f"\nRunning experiments for {stock_name}...")
        
        # Run experiments with sentiment
        print(f"Running experiments WITH sentiment analysis for {stock_name}...")
        sentiment_results = run_experiment(use_sentiment=True, stock_name=stock_name, num_trials=num_trials)
        
        # Run experiments without sentiment
        print(f"Running experiments WITHOUT sentiment analysis for {stock_name}...")
        no_sentiment_results = run_experiment(use_sentiment=False, stock_name=stock_name, num_trials=num_trials)
        
        # Save results to file
        with open(f'results/{stock_name}_results.txt', 'w') as f:
            f.write(f"Results for {stock_name}\n")
            f.write("\nWith Sentiment Analysis:\n")
            f.write(f"Average Training Accuracy: {statistics.mean(sentiment_results['train_acc']):.4f}\n")
            f.write(f"Average Testing Accuracy: {statistics.mean(sentiment_results['test_acc']):.4f}\n")
            f.write(f"Training Accuracy Std Dev: {statistics.stdev(sentiment_results['train_acc']):.4f}\n")
            f.write(f"Testing Accuracy Std Dev: {statistics.stdev(sentiment_results['test_acc']):.4f}\n")
            
            f.write("\nWithout Sentiment Analysis:\n")
            f.write(f"Average Training Accuracy: {statistics.mean(no_sentiment_results['train_acc']):.4f}\n")
            f.write(f"Average Testing Accuracy: {statistics.mean(no_sentiment_results['test_acc']):.4f}\n")
            f.write(f"Training Accuracy Std Dev: {statistics.stdev(no_sentiment_results['train_acc']):.4f}\n")
            f.write(f"Testing Accuracy Std Dev: {statistics.stdev(no_sentiment_results['test_acc']):.4f}\n")
